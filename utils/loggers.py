...
class CsvLogger:
    ...
    def write(self, args: Dict[str, Any]) -> None:
        ...
        ##
        file_list = os.listdir(base_path() + "results/" + self.setting + "/" + self.dataset \
               + "/" + args["pltf"] + "-" + self.model)
        file_list.sort()
        if len(file_list):
            file_num = int(file_list[-1].replace("mean_accs","").replace(".csv",""))+1
        else:
            file_num = 1
        ##
        ...
        path = base_path() + "results/" + self.setting + "/" + self.dataset \
               + "/" + args["pltf"] + "-" + self.model + "/mean_accs"+str(file_num)+".csv" #file_num추가함
        ...
        if self.setting == 'class-il':
            ...
            ##
            file_list = os.listdir(base_path() + "results/task-il" + "/" + self.dataset + "/" \
                   + args["pltf"] + "-" + self.model)
            file_list.sort()
            if len(file_list):
                file_num = int(file_list[-1].replace("mean_accs","").replace(".csv",""))+1
            ...
            path = base_path() + "results/task-il" + "/" + self.dataset + "/" \
                   + args["pltf"] + "-" + self.model + "/mean_accs"+str(file_num)+".csv" #file_num추가됨
            if not os.path.exists(path):
                write_headers = True
